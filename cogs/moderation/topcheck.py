...
class TopCheck(commands.Cog):
    ...
    async def topcheck_enable(self, ctx):
        if ctx.author.id != ctx.guild.owner_id:
            return await ctx.reply("<:alert:1410119045444927538> Only the **Server Owner** can enable topcheck.")
        if await self.is_topcheck_enabled(ctx.guild.id):
            return await ctx.reply("<:alert:1410119045444927538> Topcheck is already enabled for this server.")
    ...
    async def topcheck_disable(self, ctx):
        ...
        if not await self.is_topcheck_enabled(ctx.guild.id):
            return await ctx.reply("<:alert:1410119045444927538> Topcheck is not enabled for this server.")
