...
class ReasonModal(ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(description=f"**<:member:1410127604136153088> Target User:** [{self.user}](https://discord.com/users/{self.user.id})\n<:circle2:1410102626129023017> **User Mention:** {self.user.mention}\n**<a:mail:1410113350636933281> DM Sent:** {dm_status}\n**<:proton:1410107307294986370> Reason:** {reason}", color=0x000000)
...
class Mute(commands.Cog):
    ...
    async def mute(self, ctx, user: discord.Member, time: str = None, *, reason=None):
        ...
        try:
            await user.send(f"<:alert:1410119045444927538> You have been muted in **{ctx.guild.name}** by **{ctx.author}** for {duration_text}. Reason: {reason or 'None'}")
            dm_status = "Yes"
        except discord.Forbidden:
            dm_status = "No"
        except discord.HTTPException:
        ...
        embed = discord.Embed(description=f"**<:member:1410127604136153088> Target User:** [{user}](https://discord.com/users/{user.id})\n"
                                          f"<:circle2:1410102626129023017> **User Mention:** {user.mention}\n"
                                          f"<a:mail:1410113350636933281> **DM Sent:** {dm_status}\n"
                                          f"**<:proton:1410107307294986370> Reason:** {reason or 'None'}\n"
    ...
    async def mute_error(self, ctx, error):
        ...
        if isinstance(error, commands.BotMissingPermissions):
            embed = discord.Embed(title="<:alert:1410119045444927538> Access Denied", description="I don't have permission to mute members.", color=self.color)
            await ctx.send(embed=embed)
        elif isinstance(error, discord.Forbidden):
            embed = discord.Embed(title="<:alert:1410119045444927538> Missing Permissions", description="I can't mute this user as they might have higher privileges (e.g., Admin).", color=self.color)
            await ctx.send(embed=embed)
            
        else:
            embed = discord.Embed(title="<:alert:1410119045444927538> Unexpected Error", description=str(error), color=self.color)
            await ctx.send(embed=embed)
