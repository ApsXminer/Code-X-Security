...
class Warn(commands.Cog):
    ...
    async def warn(self, ctx, user: discord.Member, *, reason=None):
        ...
        try:
            ...
            embed = discord.Embed(description=f"**<:member:1410127604136153088> Target User:** [{user}](https://discord.com/users/{user.id})\n"
                                              ...
                                              f"**<:arrow_right:1410112731796865095> Reason:** {reason_to_send}\n"
                                              f"**<:star:1410101217186549800> Total Warns:** {total_warns}",
                                              color=self.color)
            embed.set_author(name=f"Successfully Warned {user.name}", icon_url=self.get_user_avatar(user))
            embed.add_field(name="<:automod2:1410103561425256539> Moderator:", value=ctx.author.mention, inline=False)
    ...
    async def clearwarns(self, ctx, user: discord.Member):
        try:
            ...
            embed = discord.Embed(description=f"<:tick:1410118593089372242> | All warnings have been cleared for **{user}** in this guild.", color=self.color)
