...
from utils.config import GUILD_JOIN_CHANNEL_ID, GUILD_REMOVE_CHANNEL_ID
...
class Guild(Cog):
    ...
    async def on_guild_add(self, guild):
        try:
            ...
            embed.add_field(
                name="**__Members__**",
                value=f"""<:members:1410120717474861106> Members : {len(guild.members)}\n<:member:1410127604136153088> Humans : {len(list(filter(lambda m: not m.bot, guild.members)))}\n<:robot:1348340531335270420> Bots : {len(list(filter(lambda m: m.bot, guild.members)))}
                """,
                inline=False
            )
            embed.add_field(
...
Threads : {len(guild.threads)}
            ...
            embed = discord.Embed(description="<:arrow_right:1410112731796865095> Prefix For This Server is `$`\n<:arrow_right:1410112731796865095> Get Started with `$help`\n<:arrow_right:1410112731796865095> For detailed guides, FAQ & information, visit our **[Support Server](https://discord.gg/pndSM8wrmS)**",
    color=0xff0000)
            embed.set_author(name="Thanks for adding me!", icon_url=guild.me.display_avatar.url)
            embed.set_footer(text="Powered by Code X Security", icon_url="https://cdn.discordapp.com/icons/699587669059174461/f689b4366447d5a23eda8d0ec749c1ba.png")
            if guild.icon:
                embed.set_thumbnail(url=guild.icon.url)
