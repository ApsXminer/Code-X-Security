...
class Nightmode(commands.Cog):
    ...
    async def nightmode(self, ctx):
        ...
        nightmode_embed.add_field(
            name="Usage",
            value="<a:dot:1410099793983967332> `nightmode enable`\n<a:dot:1410099793983967332> `nightmode disable`",
            inline=False
        )
    ...
    async def enable_nightmode(self, ctx):
        if ctx.guild.member_count < 50:  
            return await ctx.send(embed=discord.Embed(title="<:cross:1410121962189095003> Access Denied",
                color=self.color,
        ...
        if not manageable_roles:
            return await ctx.send(embed=discord.Embed(title="<:cross:1410121962189095003>  Error",
                color=self.color,
        ...
        await ctx.send(embed=discord.Embed(title="<:tick:1410118593089372242> Success",
            color=self.color,
