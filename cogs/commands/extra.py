...
tick = "<:tick:1410118593089372242>"
cross = "<:cross:1410121962189095003>"
...
class Extra(commands.Cog):
  ...
  async def serverinfo(self, ctx):
        ...
        embed.add_field(
            name="**__About__**",
            value=f"**Name : ** {ctx.guild.name}\n**ID :** {ctx.guild.id}\n**Owner <:owner:1410176722476990525> :** {ctx.guild.owner} (<@{ctx.guild.owner_id}>)\n**Created At : ** {c_at}\n**Members :** {len(ctx.guild.members)}",
            inline=False
        )
        ...
        if ctx.guild.features:
            features = "\n".join([f"<:tick:1410118593089372242>: {feature[:1].upper() + feature[1:].lower().replace('_', ' ')}" for feature in ctx.guild.features])
            embed.add_field(
                name="**__Features__**",
...
**ID:** {member.id}
**Nickname:** {nickk}
**Bot?:** {'<:tick:1410118593089372242> Yes' if member.bot else '<:cross:1410121962189095003> No'}
**Badges:** {badges}
