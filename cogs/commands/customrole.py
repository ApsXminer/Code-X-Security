...
class Customrole(commands.Cog):
                ...
                if data:
                    ...
                    if reqrole:
                        if context.author == context.guild.owner or reqrole in context.author.roles:
                            ...
                            embed = discord.Embed(title="<:alert:1410119045444927538> Access Denied",
    ...
    async def reset(self, context: Context) -> None:
        if context.author == context.guild.owner or context.author.top_role.position > context.guild.me.top_role.position:
            ...
            description=f"Deleted All Custom Role commands <:tick:1410118593089372242>\n\n**Removed Roles:**\n" + "\n".join(removed_roles) if removed_roles else "No roles were previously set.",
            color=0x000000
        )
    ...
    async def on_message(self, message: discord.Message):
        ...
        if result:
            ...
            if reqrole is None:
                await message.channel.send("<:alert:1410119045444927538> The required role is not set up in this server. Please set it up using `setup reqrole`.")
                return

            
            if reqrole not in message.author.roles:
                await message.channel.send(embed=discord.Embed(description=f"<:alert:1410119045444927538> You need the {reqrole.mention} role to use this command.", color=0x000000))
                return
            ...
            try:
                if role in member.roles:
                    ...
                    await message.channel.send(embed=discord.Embed(
                        title="<:tick:1410118593089372242> Success",
